...
In contrast, the vertical partitioning of XXX \cite{} and 2D partitioning of YYY \cite{} 
...
\rb{I rewrote the following.}
The algorithm performs asynchronous I/O, which allows the underlying file system \cite{sfs}
to merge the I/O requests from mutliple threads into larger I/Os.
The file system accepts I/Os immediately and merges them when they write contiguous
regions.
%We further use the global task queue \textit{trQ} to help merge writes to
%the output matrix on SSDs. 
Large writes ensure sustainable write throughput to SSDs and increase their duration.
To assist in I/O merging, we use
...
asynchronously with \textit{write\_rows\_async()}.
%, which postpones writes,

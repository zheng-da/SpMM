...
they require an entry for each row/column in the row/column index. We use
a very compact format to store non-zero entries and refer to this format
as SCSR (Super Compressed Row Storage) (Figure \ref{tile_format}).
...
\begin{figure}
	\begin{center}
		...
		\caption{The ratio of the storage size used by SCSR and DCSR format
		for real-world graphs.}
		\label{fig:storage}
	\end{center}
\end{figure}

SCSR is even more compact than doubly compressed sparse column (DCSC)
\cite{Buluc08}. In the best case (each non-zero entry is stored in a separate
row/column), SCSR requires only half of the space to store locations of
non-zero entries than DCSC. Figure \ref{fig:storage} shows that SCSR requires
only 60\%-70\% of the storage size for many real-world graphs. A compact format
is critical to performance because a sparse matrix is read from SSDs in
semi-external memory and SSDs may be the bottleneck in the operation.

...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State {$trQueue$:} \Comment{a queue of tile rows shared by all threads}
		\While{$|trQueue| > 0$}
		\If {$|trQueue| > \#threads$}
		...
		\If {more than 4 pending I/Os} \State wait for I/O
		\EndIf
		\EndWhile
		\EndProcedure
		...
		\Procedure{ProcessSTRow}{$trs$, $inMat$}
		\State {$trs$:} \Comment{$s \times n$ submatrix in the sparse matrix}
		\State {$inMat$:} \Comment{$n \times p$ input dense matrix of SpMM}

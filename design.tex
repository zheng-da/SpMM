...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Statex \textbf{Input}: $outM$, a $n \times p$ output dense matrix on SSDs
		\While{$|\vec{trQ}| > 0$}
		\If {$|\vec{trQ}| > \#threads$}
...
ensures that all threads are processing contiguous tile rows. As such,
available computation results in memory are spatially close to each other
on SSDs to help I/O merge.

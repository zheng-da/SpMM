...
and invokes \textit{ProcessSTRow} to process the tile rows once I/O is complete.
...
then further splits a super tile into tiles with $get\_tiles$. It processes
all tiles in a super tile
...
is complete, the local buffer contains the complete computation results and
is written back to SSDs asynchronously. The computation result from a task
may be small. Instead of writing the result immediately, $write\_async()$ may
postpone the write and merge it with writes from other threads before writing
them to SSDs with a single I/O.

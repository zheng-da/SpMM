...
To reduce the overall storage size of a sparse matrix, we need a very compact format
to store non-zero entries in a tile. A compact format is critical to performance
because in semi-external memory SpMM a sparse matrix is read from SSDs and SSDs
...
and is compact for hypersparse matrices, especially for submatrices of
a sparse matrix. However, even DCSC wastes space because it requires space
for pointers to columns and \dz{fails to reference to non-zero values with small
integers}.
...
the matrix. Because the most significant bit is used to indicate the beginning
of a row, this format allows a maximum tile size of $32K \times 32K$. SCSR is
...
than DCSC for binary sparse matrices. Figure \ref{fig:storage} shows that
in practice SCSR uses 45\%-70\% of the storage size used by DCSC for large
real-world graphs (Table \ref{graphs}).

\begin{figure}
	\begin{center}
		\footnotesize
		\include{SpMM_figs/storage}
		\caption{The ratio of the storage size required by SCSR and DCSR
			\cite{Buluc08} format for real-world graphs. SCSR is much more
		compact than DCSR for graphs.}
		\label{fig:storage}
	\end{center}
\end{figure}
...
At the beginning, a thread gets larger computation tasks (one super tile row
...
A super tile is composed of tiles from multiple contiguous tile rows and its
...
It splits a super tile row into super tiles with $get\_super\_tiles$ and
then further into tiles with $get\_tiles$. It processes all tiles in
a super tile before moving to the next one, to reuse data in the CPU cache from
the computation on the previous super tile. For each super tile row, a thread
maintains a local memory buffer $outBuf$ to store the computation results,
which minimizes remote memory access. Once the computation on a super tile row
is complete, the local buffer contains complete results and is written back to

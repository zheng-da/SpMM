...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Statex \textbf{Input}: $spm$, a $n \times n$ sparse matrix
		\Statex \textbf{Input}: $inM$, a $n \times p$ dense matrix
		\Statex \textbf{Output}: $outM$, a $n \times p$ dense matrix
		\State $trQ \gets get\_tile\_row\_locs(spm)$
		\State $outM \gets zeros(n, p)$
		\ParFor{$j \gets 1, \#threads$}
		\State $ProcessTileRows(spm, trQ, inM, outM)$
		\EndParFor
		\EndProcedure

		\State

		\Procedure{ProcessTileRows}{$spm$, $trQ$, $inM$, $outM$}
		\Statex \textbf{Input}: $spm$, a $n \times n$ sparse matrix
		\Statex \textbf{Input}: $trQ$, a queue that contains the location of all tile rows
		\Statex \textbf{Input}: $inM$, a $n \times p$ input dense matrix of SpMM
		\Statex \textbf{Input}: $outM$, a $n \times p$ output dense matrix of SpMM
		\While{$|trQ| > 0$}
		\If {$|trQ| > \#threads$}
		\State $locs \gets get\_super\_tile\_row(trQ)$
		\Else \State $locs \gets get\_tile\_row(trQ)$
		\EndIf
		\State $tr \gets read\_tilerow\_async(spm, locs)$
		\State $res \gets ProcessSTRow(tr, inM)$ if $tr$ is ready
...
For parallel computation, we construct a global task queue \textit{trQueue}

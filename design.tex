...
\dz{describe the hardware architecture?}
...
stream the output matrix to SSDs during the computation if necessary. We stream
...
dense matrix in memory for many of today's machines. If a machine has insufficient
memory to keep the output dense matrix, we stream the output dense matrix
back to SSDs during the computation. Another option is to stream the data
...
we partition the input dense matrix vertically (Figure \ref{dense_mat}) so
...
To better support access to multiple dense matrices on SSDs simultaneously, SAFS
stripes data in a dense matrix across SSDs with a different striping order.
...
potentially reduce write amplification on SSDs \cite{Tang15}, SAFS stripes data
...
I/O access. Therefore, SAFS generates a random striping order for each dense
matrix to evenly distribute I/O among SSDs. SAFS stores the striping order
...
of the sparse matrix in sparse matrix multiplication. \dz{To be complete.}

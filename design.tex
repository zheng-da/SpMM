...
\begin{algorithm}
	...
	\label{alg:spmm}
	\begin{algorithmic}[1]
		\Procedure{ProcessRowTiles}{TRQueue}
		\While{$|TRQueue| > 0$}
		\If {$|TRQueue| > thres$} \State $NumTRs \gets STileSize$
		\Else \State $NumTRs \gets 1$
		\EndIf
		\State $STR \gets get\_tile\_rows(TRQueue, NumTRs)$
		\State $read\_async(STR, callback=ProcessSTRow)$
		\If {$pendingIOs > maxIOs$} \State wait for I/O
		\EndIf
		\EndWhile
		\EndProcedure

		\Procedure{ProcessSTRow}{STR}
		\State $STRowIdx \gets get\_start\_row(STR)$
		\State $outBuf \gets matrix(0, nrow(tile), ncol(inMat))$
		\State $STs \gets get\_super\_tiles(STR)$
		\For{$ST \in STs$}
		\State $tiles \gets get\_tiles(ST)$
		\For{$tile \in tiles$}
		\State $colIdx \gets get\_start\_col(tile)$
		\State $numCols \gets get\_num\_cols(tile)$
		\State $rowIdx \gets get\_start\_row(tile)$
		\State $lRowIdx \gets rowIdx - STRowIdx$
		\State $numRows \gets get\_num\_rows(tile)$
		\State $lInMat \gets get\_rows(inMat, colIdx, numCols)$
		\State $lres \gets tile * lInMat$
		\State $write\_rows(outBuf, lRowIdx, numRows, lres)$
		\EndFor
		\EndFor
		\State $write\_async(outMat, lOutMat)$
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
We partition a sparse matrix horizontally into tile rows for parallelization
(Figure
...
%, which is essential for semi-external memory computation because
...
For parallel computation, each thread runs \textit{ProcessRowTiles} (in Algorithm
\ref{alg:spmm}) that gets computation tasks (tile rows) from a global task queue
\textit{TRQueue}, which results in good load balancing and large I/O writes
to SSDs. The global task queue is the only data structure that incurs
concurrent writes from threads. At the beginning, a thread gets larger
computation tasks (one super tile row at a time); when the computation
approaches completion, a thread gets one tile row at a time. This design reduces
concurrent access to the global data structure while achieving good load balancing.
When a thread gets a computation task, it reads tile rows asynchronously.
Once I/O is complete, \textit{ProcessSTRow} is invoked to process the tile rows.
$get\_tile\_rows()$ controls a global execution order, which is essential
to increase large I/O writes to SSDs. When a thread gets
...
To better utilize the CPU cache, we process tiles of a partition in super tiles.
The tile size of a sparse
...
A super tile is composed of tiles from multiple tile rows and its
...
\textit{ProcessSTRow} in Algorithm \ref{alg:spmm} processes a super tile row.
$get\_super\_tiles$ splits a super tile row into super tiles and $get\_tiles$
further splits super tile into tiles. We process all tiles in a super tile
before moving to the next super tile, to reuse data in the CPU cache from
the computation for the previous super tile. For each super tile row, a thread
maintains a local memory buffer $outBuf$ to store the computation results,
which minimizes remote memory access.
Once the computation on a super tile row is complete, the local buffer
contains the complete computation results and is written back to SSDs
asynchronously.
...
the input dense matrix and add the results to the corresponding row of

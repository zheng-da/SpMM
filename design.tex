...
CPU cache hits and reduce the amount of data read from SSDs. Compressed row
storage (CSR) or compressed column storage (CSC) format are not designed for
graphs because these formats incur many CPU cache misses.
...
specified by applications. To handle dense matrices with different numbers
of columns, we deploy both static cache blocking and dynamic cache blocking.
As such, we generate sparse matrices with a relatively small tile size and
...
Streaming the sparse matrix from SSDs results in no cache hits in the Linux
page cache when the sparse matrix size is larger than main memory. As such,
we access data of the sparse matrix on SSDs with direct I/O.

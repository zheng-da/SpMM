...
Another format, Sparsity \cite{Im04}, encodes both register blocking and cache blocking to
...
size of a 2D-partitioned sparse matrix. Our format further reduces the storage size of a sparse
matrix.  SciDB \cite{} developed a general purpose dense array format that includes 2-d and 
3-d partitioning and ghost cells \cite{}.  It is not well-suited to sparse matrixes.
...
%Array database literature. \dz{scidb}
...
to SSDs.  GridGraph runs much more slowly than in-memory engines and our 
SEM SpMM implementation.
 \rb{Why do we not compare with GridGraph?  Should we?  This treatment
leaves many questions.  Is it slow? I wrote the above paragraph assuming that it's correct.}

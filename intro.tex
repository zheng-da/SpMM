...
multiplication. This operation has very low computation density and its performance
is limited by memory access. As such, it usually achieves only
a small fraction of the peak performance of a modern processor \cite{Williams07}.

It is even more challenging to perform this operation on sparse matrices
encoding real-world graphs. Graphs usually have nearly
random vertex connection, which leads to many random memory access, and
power-law distribution in vertex degree, which causes load imbalance.
Furthermore, many real-world graphs are enormous. For example,
Facebook's social network has billions of vertices and today's Web graphs
are even much larger. These graphs are also very sparse. As such, the dense
matrices involved in sparse matrix multiplication are also very large.
...
\begin{itemize}
	...
	\item We deploy a very compact sparse matrix format to reduce the amount of I/O
		and alleviate I/O as the bottleneck of the system.
	\item We explore additional runtime optimizations on SpMM that meet memory
		constraints in semi-external memory to reduce both CPU cache misses and
		remote memory access in a NUMA machine.
	\item We optimize SpMM for dense matrices with various numbers of columns and
		extends SEM-SpMM to large dense matrices that cannot fit in memory by
		vertically partitioning the dense matrices.
\end{itemize}

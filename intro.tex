...
multiplication implementation on SSDs to achieve performance comparable to in-memory
counterparts. Even though SSDs have high IOPS, sequential I/O is preferred
because it sequential I/O throughput is significantly higher than random I/O
\cite{safs}. In addition, random writes are harmful to SSDs \cite{sfs} because
they decrease I/O throughput and shorten the lives of SSDs. Furthermore,
SSDs are an order of magnitude slower than DRAM in throughput.
...
solution for large-scale sparse matrix multiplication. It can also serve

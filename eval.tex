...
\begin{figure}
	\begin{center}
		\footnotesize
		\include{SpMM_figs/spmm.im.vs.sem}
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\include{SpMM_figs/SpMV-awesomer}
		\vspace{-10pt}
		\caption{SpMV}
		\label{perf:spmv}
	\end{subfigure}
	\begin{subfigure}[b]{0.5\textwidth}
		\centering
		\include{SpMM_figs/SpMM-awesomer}
		\vspace{-10pt}
...
\begin{figure}
	\begin{center}
		\footnotesize
		\include{SpMM_figs/spmm-32cols-overhead}
...
\begin{figure}
	\begin{center}
		\footnotesize
		\include{SpMM_figs/SpMM_optimize}
...
\begin{itemize} \itemsep1pt \parskip0pt \parsep0pt
	\item use SCSR to reduce the number of bits read from SSDs (SCSR),
...
The I/O optimizations lead to substantial speedup over the base implementations,
but behave very differently on these two graphs (Figure \ref{perf:spmm_opt_io}).
On an unclustered graph, SCSR results in much more compact storage format
than DCSR (Figure \ref{fig:storage}) and thus achieves significant speedup.
The Page graph, on the other hand, is well clustered and DCSR already
achieves a small storage size. SCSR further reduces storage size, but is less
significant. SpMV on the Page graph has less random memory access and is
I/O-bound even on a large SSD array. Therefore, \textit{Buf-pool} and
\textit{IO-poll} are very effective. In contrast, SEM-SpMV on the Friendster
graph with SCSR already achieves almost 80\% of IM-SpMV and thus, further
...
\begin{figure}
	\begin{center}
		\footnotesize
		\include{SpMM_figs/eigen-runtime-8ev}
